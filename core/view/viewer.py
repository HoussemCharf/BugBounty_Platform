	...
	def render_template(view,session=session,static_vars=static_vars,error=None,success=None,**kwargs):
		...
		if session.get('log_in') != None: 
			if session['log_in'] == True:
				...
				connecteduser = username[:2]
				is_admin = User.is_admin(_id)
				email = User.get_email_by_id(_id)
		return render_template(view,session=session,static_vars=static_vars,username=username,connecteduser=connecteduser,admin=is_admin,email=email,error=error,success=success,**kwargs)